...
from popularity_sampling import (
    PopularitySamplingElicitation,
    PopularitySamplingFromBucketsElicitation,
)
...
from mandate_allocation.exactly_proportional_fuzzy_dhondt_2 import (
...
def load_data(loader, elicitation, elicitation_movies):
    data = elicitation.get_initial_data(
        [int(x["movie_idx"]) for x in elicitation_movies]
    )
...
def load_data_1(elicitation_movies):
    ...
    data = MultiObjectiveSamplingFromBucketsElicitation(
        loader.rating_matrix, loader.similarity_matrix, 2, 2, 2, 4, 1.0
    ...
    res_genres = [
        loader.movies_df_indexed.loc[movie_id].genres.split("|")
    ...
    result = [
        ...
        for movie, url, movie_idx, genres, movie_id in zip(
            res, res_url, data, res_genres, movie_ids
        )
    ]
...
def load_data_2(elicitation_movies):
    ...
    # Get list of items
    data = PopularitySamplingElicitation(
        loader.rating_matrix, n_samples=16
    ).get_initial_data([int(x["movie_idx"]) for x in elicitation_movies])
    ...
    res_url = [loader.get_image(movie_idx) for movie_idx in data]
    result = [
        {"movie": movie, "url": url, "movie_idx": str(movie_idx)}
        for movie, url, movie_idx in zip(res, res_url, data)
    ]
    # Result is a list of movies, each movie being a dict (JSON object)
    return result
...
def load_data_3(elicitation_movies):
    ...
    data = PopularitySamplingFromBucketsElicitation(
        loader.rating_matrix, 5, [4] * 5
    ).get_initial_data([int(x["movie_idx"]) for x in elicitation_movies])
    res = [loader.movie_index_to_description[movie_idx] for movie_idx in data]
    res_url = [loader.get_image(movie_idx) for movie_idx in data]
    result = [
...
def calculate_weight_estimate(selected_movies, elicitation_movies):
    ...
    if type(elicitation_movies[0]) == int:
        ...
        movie_indices = np.unique(
            [int(movie["movie_idx"]) for movie in elicitation_movies]
        )
    else:
        assert False
    ...
    novelties = 1.0 - (
        loader.rating_matrix.astype(bool).sum(axis=0) / loader.rating_matrix.shape[0]
    )
    ...
    for movie_idx in movie_indices:
        if movie_idx in selected_movies:
            ...
            d = distance_matrix[movie_idx][movie_indices].sum() / (
                len(movie_indices) - 1
            )
            selected_diversities.append(d[d > 0].reshape(-1, 1))
            selected_novelties.append(
                1.0
                - (
                    loader.rating_matrix[:, movie_idx].astype(bool).sum()
                    / loader.rating_matrix.shape[0]
                )
            )

    if (
        (not selected_relevances)
        or (not selected_diversities)
        or (not selected_novelties)
    ):
...
def prepare_wrapper_once():
    ...
    for movie_id, row in loader.movies_df_indexed.iterrows():
        movie_title_to_idx[bytes(row.title, "UTF-8")] = loader.movie_id_to_index[
            movie_id
        ]
...
def enrich_results(top_k, loader):
    ...
    if type(loader) is MLDataLoader:
        ...
        top_k_description = [
            loader.movies_df_indexed.loc[movie_id].title for movie_id in top_k_ids
        ]
        top_k_genres = [
            loader.movies_df_indexed.loc[movie_id].genres.split("|")
            for movie_id in top_k_ids
        ]
        ...
        top_k_description = [
            loader.items_df_indexed.loc[movie_id].title for movie_id in top_k_ids
        ]
        if hasattr(loader.items_df_indexed, "genres"):
            top_k_genres = [
                loader.items_df_indexed.loc[movie_id].genres.split("|")
                for movie_id in top_k_ids
            ]
        else:
    ...
    return [
        ...
        for movie, url, movie_idx, movie_id, genres in zip(
            top_k_description, top_k_url, top_k, top_k_ids, top_k_genres
        )
    ]
...
def rlprop(selected_movies, model, weights, filter_out_movies=[], k=10):
    ...
    loader, wrapper = prepare_wrapper(
        selected_movies,
        model,
        exactly_proportional_fuzzy_dhondt_2,
        obj_weights,
...
def gen_dense_rating_matrix(rating_matrix):
    ...
    assert (
        dense_rating_matrix.ndim == rating_matrix.ndim
    ), f"Dense rating matrix should preserve ndim: {dense_rating_matrix.shape}"
    assert np.all(
        dense_rating_matrix.shape <= rating_matrix.shape
    ), f"Making dense rating matrix should not increase dimensions: {dense_rating_matrix.shape} vs. {rating_matrix.shape}"
...
def most_relevant_movies(
    group, movie_to_group, deny_list, tag, tag_counts_per_movie, loader
):
    movie_counts = dict()
    for movie, tag_counts in tag_counts_per_movie.items():
        if (
            movie in deny_list
            or movie not in movie_to_group
            or movie_to_group[movie] != group
        ):
...
def label_groups(group_assignment, tags, tag_counts_per_movie):
    ...
    for item, group in group_assignment.items():
        ...
        for tag in tags:
            if (
                tag in tag_counts_per_movie[item]
                and tag_counts_per_movie[item][tag] > 0
            ):
    ...
    for group in set(group_assignment.values()):
        ...
        for tag in tags_per_group[group]:  # tags:
            if tag in tag_deny_list:
                ...
                d1 = acc_per_cluster_tag_relevance(
                    tag, group_to_items, tag_counts_per_movie
                )
                if d1 == 0:
                    uniqueness = 0.0
                else:
                    uniqueness = (
                        tag_relevance(tag, group_to_items[group], tag_counts_per_movie)
                        / d1
                    )
                d2 = acc_per_tag_tag_relevance(
                    group_to_items[group], tag_counts_per_movie
                )
                if d2 == 0:
                    relevance = 0.0
                else:
...
def search_for_movie(attrib, pattern, tr=None):
    ...
    if tr:
        found_movies = loader.movies_df[
            loader.movies_df.movieId.astype(str)
            .map(tr)
            .str.contains(pattern, case=False)
        ]
    else:
        found_movies = loader.movies_df[
            loader.movies_df.title.str.contains(pattern, case=False)
        ]

    movie_indices = [
        loader.movie_id_to_index[movie_id] for movie_id in found_movies.movieId.values
    ]
    res_url = [loader.get_image(movie_idx) for movie_idx in movie_indices]
    result = [
        {"movie": movie, "url": url, "movie_idx": str(movie_idx)}
        for movie, url, movie_idx in zip(
            found_movies.title.values, res_url, movie_indices
        )
    ]
    return result
